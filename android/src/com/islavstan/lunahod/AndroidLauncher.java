...
public class AndroidLauncher extends AndroidApplication {
	...
	protected void onCreate(Bundle savedInstanceState) {
       ...
       //отображение окна без заголовка и на полный экран
		...
		//инициализируем главный экран игры
		View gameView = initializeForView(new Lunahod(gameCallback));
	...
	private GameCallback gameCallback = new GameCallback() {
		...
		public void sendMessage(int message) {
			...
			if (message == Lunahod.SHOW_BANNER) {
				AndroidLauncher.this.runOnUiThread(new Runnable() {
					@Override
					public void run() {
					}
				});
			} else if (message == Lunahod.HIDE_BANNER) {
				AndroidLauncher.this.runOnUiThread(new Runnable() {
					@Override
					public void run() {
					}
				});
			} else if (message == Lunahod.LOAD_INTERSTITIAL) {
				// подгрузка межстраничного баннера


			} else if (message == Lunahod.SHOW_INTERSTITIAL) {
				AndroidLauncher.this.runOnUiThread(new Runnable() {
					@Override
					public void run() {
			...
			} else if (message == Lunahod.OPEN_MARKET) {
				Uri uri = Uri.parse(getString(R.string.share_url));
				Intent intent = new Intent(Intent.ACTION_VIEW, uri);
				startActivity(intent);


			} else if (message == Lunahod.SHARE) {
